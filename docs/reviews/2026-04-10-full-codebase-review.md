...
- **Pagefind migration plan**: The Component UI uses web components (`<pagefind-modal>`, `<pagefind-searchbox>`) instead of `new PagefindUI()`. CSS customization moves from `.pagefind-ui__*` class overrides to CSS custom properties (`--pagefind-ui-primary`, `--pagefind-ui-text`, etc.) via shadow DOM. Files are auto-generated by Pagefind v1.5.0. Migration steps: (1) swap CSS/JS imports to `pagefind-component-ui.*`, (2) replace `<div id="search">` + JS init with `<pagefind-searchbox>` or `<pagefind-modal>` web component, (3) replace all `#search .pagefind-ui__*` CSS overrides with CSS custom properties, (4) test search functionality. Medium effort, best done in a focused session.

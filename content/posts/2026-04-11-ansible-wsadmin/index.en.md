...
I started where everyone starts: parsing AdminConfig output with regex. March 10: [*strengthen the parser on corner cases...*](https://github.com/vjt/ansible-wsadmin/commit/895510c). March 20: [*fix parsing of single-item lists and anonymous objects*](https://github.com/vjt/ansible-wsadmin/commit/d404a9d). You can hear the frustration in the commit messages — each fix uncovered a new corner case where AdminConfig's string format was ambiguous or self-contradictory.

By March 25 I was already looking at the JMX layer: [*abstract jmx invoke and app info parsing*](https://github.com/vjt/ansible-wsadmin/commit/0f90df4). I used IntelliJ IDEA to decompile the WAS bytecode and traced how AdminConfig's own code worked — turns out it was calling ConfigService internally and then converting the results to strings. All that parsing I was fighting was reconstructing what ConfigService had already returned as proper Java objects.

March 27, 4 AM: [*wip: remove regex-based parsing, move down one layer*](https://github.com/vjt/ansible-wsadmin/commit/9eb4e21). The pivot. The old parser gets demoted to [configparser.py](https://github.com/vjt/ansible-wsadmin/blob/master/lib/wsadmin/lib/configparser.py) (still used for the few corners where AdminTask is unavoidable), and [activeconfig](https://github.com/vjt/ansible-wsadmin/tree/master/lib/wsadmin/lib/activeconfig) is born: [*move legacy wrapper in configparser, implement new object-oriented activeconfig wrapper*](https://github.com/vjt/ansible-wsadmin/commit/2ab046b).

48 hours later: [*extend activeconfig to a true object oriented ORM for configservice*](https://github.com/vjt/ansible-wsadmin/commit/4633247). March 31: [*finalise object oriented traversal*](https://github.com/vjt/ansible-wsadmin/commit/f724b6b). The string parser had been taking weeks of incremental fixes. The ConfigService-based replacement took a weekend.

The daemon showed up even earlier — March 14: [*EXPERIMENTAL - add wsadmin server to speed things up dramatically*](https://github.com/vjt/ansible-wsadmin/commit/c95b970). The next day: [*use the server for all tasks yippee*](https://github.com/vjt/ansible-wsadmin/commit/f5f2255).

...
The database layer was a proper driver interface — a struct of function pointers that abstracted away the database engine:
...
Same for the hash functions (`stolen from bahamut/src/hash.c`) and the pattern matching code (`stolen from bahamut/src/match.c`). Every borrowed piece was credited in a comment.
...
In the `tools/` directory sits `netxplode.pl` — "The Network Daemon Exploder" by Daniel Dent. A Perl script that spawns 100 IRC clients and hammers the services with random commands:

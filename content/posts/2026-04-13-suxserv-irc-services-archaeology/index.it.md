...
Ma IRC era una giungla. Il [protocollo originale](https://www.rfc-editor.org/rfc/rfc1459) non aveva il concetto di identità persistente — [niente registrazione di nickname, niente proprietà dei canali](https://www.irchelp.org/networks/noserv.html). Chi si connetteva per primo e sceglieva un nickname lo possedeva, finché restava online. Disconnettersi un secondo e qualcun altro poteva prenderselo. La gente usava BNC — bouncer, processi proxy su server permanentemente connessi — solo per tenere il proprio nickname mentre dormiva. Era furto d'identità in piena regola, e la tua unica difesa era una macchina che probabilmente non potevi permetterti di tenere online 24/7.

Ma c'era di peggio. Le reti IRC erano [federazioni di server](https://daniel.haxx.se/irchistory.html), e i server a volte perdevano il contatto tra loro — un [*netsplit*](https://en.wikipedia.org/wiki/Netsplit). Durante uno split, la rete si spezzava in isole. Potevi connetterti a un'isola e prendere il nickname di qualcuno, e quando i server si riunivano, entrambi gli utenti avevano lo stesso nick. Il protocollo gestiva la cosa con terra bruciata: ogni istanza del nickname in collisione veniva [uccisa](https://en.wikipedia.org/wiki/IRC_takeover) — *entrambi* gli utenti disconnessi. L'attaccante, pronto all'evento, si riconnetteva immediatamente. La vittima tornava e trovava il suo nick rubato.
...
ChanServ faceva lo stesso per i canali — registri, configuri le access list, e il service manteneva l'ordine anche quando non eri online. Questo dava agli utenti normali un modo per tenere al sicuro la propria identità senza bisogno di una macchina permanentemente connessa a internet.
...
Quindi: cosa sono i services, tecnicamente? Gli pseudo-utenti che gestiscono la burocrazia di una rete IRC: **NickServ** registra e protegge i nickname, **ChanServ** gestisce la proprietà dei canali e le access list, **MemoServ** consegna messaggi offline, **OperServ** dà agli operatori di rete strumenti amministrativi, e **RootServ** gestisce le operazioni in god-mode.

Non fanno parte del server IRC in sé. Si connettono alla rete come un server speciale, parlando il protocollo server-to-server — ricevendo così tutte le informazioni sugli utenti che si connettono, disconnettono, entrano e escono dai canali da tutti gli altri server — e si presentano come utenti virtuali. Quando scrivi `/msg NickServ IDENTIFY miapassword`, stai parlando a un service — un processo separato, con il suo database, il suo parser di protocollo, la sua state machine che traccia ogni utente e canale della rete.

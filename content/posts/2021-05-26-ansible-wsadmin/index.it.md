...
Quello che ho trovato ha cambiato il mio approccio all'intero progetto.
...
**2003 — WAS 5.0: La riscrittura.** IBM riscrive WebSphere da zero. La configurazione si sposta da un database a file XML. Il framework di amministrazione JMX arriva con due layer: **ConfigService** (l'MBean — oggetti Java tipizzati, interfaccia coerente) e **AdminConfig/AdminControl/AdminApp** (wrapper di scripting — tutto è stringhe). Il layer di scripting ha come target [JACL](https://en.wikipedia.org/wiki/Tcl_Java) (Tcl-in-Java), il che spiega l'ossessione per le stringhe: Tcl è un linguaggio orientato alle stringhe. Ma poi IBM aggiunge il supporto Jython in WAS 5.1 (2004), e l'API a stringhe ha ancora meno senso.

**2006 — WAS 6.1: Arriva AdminTask.** Un quarto oggetto di scripting, progettato per fornire comandi "task-oriented" di convenienza. Invece di semplificare le cose, aggiunge un altro livello di indirezione con la sua propria sintassi per gli argomenti: `AdminTask.createSIBJMSQueue('...(cells/...)', '[-name Queue -jndiName jms/Queue -busName Bus]')`. È una stringa contenente coppie chiave-valore dentro parentesi dentro una stringa. La documentazione IBM per AdminTask è centinaia di pagine di queste formule magiche.

**2008 — WAS 7.0: Script Libraries.** IBM distribuisce funzioni Jython pre-costruite organizzate per categoria. Un passo nella direzione giusta, ma costruite sempre sul layer AdminConfig a stringhe.

### La risposta della community

La parte rivelatrice non è che la community ha costruito wrapper. È *chi* ha costruito wrapper.

**[wsadminlib.py](https://github.com/wsadminlib/wsadminlib)** (2006, pubblicato nel 2010) è stato creato da **due sviluppatori di prodotto IBM** che, parole loro, *"were having trouble understanding the native wsadmin syntax"* — "avevano difficoltà a capire la sintassi nativa di wsadmin." Gli stessi ingegneri di IBM non riuscivano a usare la propria API. La libreria è cresciuta fino a 500+ metodi con contributi da 30+ ingegneri IBM in tutto il mondo. La loro [presentazione al WASUG 2011](https://www.slideshare.net/kelapure/wsadminlibwasug2011-01250726) è una masterclass di critica diplomatica del prodotto del tuo stesso datore di lavoro: funzione dopo funzione che "nasconde le assurdità sottostanti".

**[WDR](https://github.com/WDR/WDR)** (WebSphere Deployment Robot) prende un approccio diverso: wrappa l'output di AdminConfig in oggetti Python per poter fare accesso a proprietà invece di parsing di stringhe. Il loro pitch: *"Lists must be treated as lists, no more splitline()"* — "le liste devono essere trattate come liste, basta splitline()." Che questo debba essere un punto di vendita dice tutto sull'esperienza di base.

**[myarch.com](https://myarch.com/getting-started-wsadmin/)** documenta l'API con una franchezza che la documentazione ufficiale IBM non raggiunge mai. La loro descrizione delle strutture dati di AdminTask come *"truly baffling"* — "davvero sconcertanti" — e delle *"ten thousand options"* di AdminApp.install() cattura la frustrazione collettiva della community. Hanno costruito il loro [framework di automazione](https://myarch.com/was-automation) con un DSL dichiarativo.

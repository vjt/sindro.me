...
**2002 — WAS 5.0: La riscrittura.** IBM [riscrive WebSphere da un codebase comune](https://en.wikipedia.org/wiki/IBM_WebSphere_Application_Server#702). Il [repository di configurazione basato su database viene sostituito da file XML](https://en.wikipedia.org/wiki/IBM_WebSphere_Application_Server#702), gestiti da un Deployment Manager che replica sui nodi. [wsadmin sostituisce il vecchio tool WSCP](https://en.wikipedia.org/wiki/Wsadmin). Il framework di amministrazione JMX arriva con due layer: **ConfigService** (l'MBean — oggetti Java tipizzati, interfaccia coerente) e **AdminConfig/AdminControl/AdminApp** (wrapper di scripting — tutto è stringhe). Il layer di scripting ha come target [JACL](https://en.wikipedia.org/wiki/Tcl_Java) (Tcl-in-Java), il che spiega l'ossessione per le stringhe: Tcl è un linguaggio orientato alle stringhe. Ma poi IBM aggiunge il supporto Jython in WAS 5.1 (2004), e l'API a stringhe ha ancora meno senso.

**2006 — WAS 6.1: Arriva AdminTask.** Un [quarto oggetto di scripting](https://www.ibm.com/docs/en/was/8.5.5?topic=scripting-admintask-object-wsadmin), progettato per fornire comandi "task-oriented" di convenienza. Invece di semplificare le cose, aggiunge un altro livello di indirezione con la sua propria sintassi per gli argomenti: `AdminTask.createSIBJMSQueue('...(cells/...)', '[-name Queue -jndiName jms/Queue -busName Bus]')`. È una stringa contenente coppie chiave-valore dentro parentesi dentro una stringa. La [documentazione IBM per AdminTask](https://www.ibm.com/docs/en/was/8.5.5?topic=scripting-admintask-object-wsadmin) è centinaia di pagine di queste formule magiche. La console di admin guadagna "Command Assistance" — mostra l'equivalente wsadmin delle azioni della console — il che evidenzia solo quanto siano contorti i comandi.

**2008 — WAS 7.0: Script Libraries.** IBM distribuisce [funzioni Jython pre-costruite](https://www.ibm.com/docs/en/was/8.5.5?topic=scripting-script-library) organizzate per categoria (Application, Resources, Security, Servers, System). Un passo nella direzione giusta, ma costruite sempre sul layer AdminConfig a stringhe.

### La risposta della community

La parte rivelatrice non è che la community ha costruito wrapper. È *chi* ha costruito wrapper.

**[wsadminlib.py](https://github.com/wsadminlib/wsadminlib)** (usato internamente in IBM dal 2006, [pubblicato su developerWorks nel 2010](https://wsadminlib.blogspot.com/)) è stato creato da sviluppatori di prodotto IBM per [fornire oltre 500 metodi con nomi intuitivi e parametri per sostituire i complessi comandi AdminConfig](https://www.slideshare.net/kelapure/wsadminlibwasug2011-01250726). Fermatevi un secondo a rifletterci: gli stessi ingegneri di IBM hanno costruito un wrapper da 500 metodi perché l'API che hanno distribuito aveva bisogno di essere sostituita. La [presentazione al WASUG 2011](https://www.slideshare.net/kelapure/wsadminlibwasug2011-01250726) di Rohit Kelapure attraversa metodo dopo metodo progettato per "nascondere la sintassi" — il modo diplomatico per dire che la sintassi non dovrebbe essere vista da esseri umani.
